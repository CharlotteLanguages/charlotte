...
export function openFormClasses(btnshow, btnclose, modal, table) {
  d.addEventListener("click", (e) => {
    if (e.target.matches(btnshow)) {
      ...
      d.querySelector(".btn-calendar-class").classList.toggle(
        "btn-calendar-visible"
      );
    ...
    if (e.target.matches(btnclose)) {
      ...
      d.querySelector(".btn-calendar-class").classList.remove(
        "btn-calendar-visible"
      );
      d.querySelector(".tooltip").classList.remove("show_tooltip");
...
const renderClasses = (clase) => {
  ...
  clase.reverse().forEach((ele) => {
    ...
    <td data-label = "Date">${ele.classSetData
      .split("-")
      .reverse()
      .join("-")} ${ele.classSetTime}</td>
    ...
    d.addEventListener("submit", async (e) => {
      if (e.target === $formDelete) {
        ...
        let options = {
            ...
            body: JSON.stringify({
              reasonDeleteClasses: e.target.reasonDelete.value,
            }),
          },
          res = await fetch(
            "http://localhost:3000/reasonDeleteClasses",
            options
          ),
          json = await res.json();
      }
...
d.addEventListener("click", (e) => {
  ...
  if (e.target.matches(".btn-calendar-class")) {
    ...
    d.querySelector(".cont-tables-classes").classList.add(
      "up-table-certificate"
    );
    //remover
