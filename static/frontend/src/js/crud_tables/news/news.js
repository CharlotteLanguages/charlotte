...
const renderNewsData = (news) => {
  news.reverse().forEach((ele) => {
    ...
    <tbody class = "body">
    <tr class = "tr">
    <td data-label = "ID">${ele.idNews}</td>
    <td data-label = "News">${ele.title}</td>
    <td data-label = "Category">${ele.category}</td>
    <td data-label = "Tags">${ele.tags}</td>
    <td data-label = "Actions">
        <div class="icons-news">
        <i class="fas fa-dot-circle read-news" data-ids=${ele.idNews}></i>
        <i class="fas fa-pen edit-news" data-id = ${ele.idNews}></i> 
        <i class="fas fa-times-circle remove-news" data-idr =${ele.idNews}></i>
        </div>
    </td>
    </tr>
    </tbody>`;
      ...
      try {
        let options = {
            ...
            body: JSON.stringify({
              ...
              category: e.target.categoryNews.value,
              tags: e.target.tagsNews.value,
              image:e.target.imageNews.value,
      ...
      if (e.target === $formDelete) {
        ...
        try {
          let options = {
              ...
              headers: { "Content-type": "application/json" },
              body: JSON.stringify({
                reasonDeleteNews: e.target.reasonDeleted.value,
              }),
            },
            res = await fetch("http://localhost:3000/reasonDeleteNews", options),
            json = await res.json();
